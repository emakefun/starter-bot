#ifndef _STARTERBOT_H_
#define _STARTERBOT_H_
#include <stdint.h>
#include "Arduino.h"
#include "SmartCar.h"
#include "IR_remote.h"
...
#define EM_ECHO_PIN 3
#define EM_TRIG_PIN 2
...
class StarterBot : public SmartCar {
private :
    ...
    uint8_t EchoPin,Trig_Pin,ServoPin;
    ST_PROTOCOL SendData;
    ProtocolParser *mProtocolPackage;

public :
    StarterBot(ProtocolParser *Package, uint8_t in1 = EM_IN1_PIN, uint8_t in2 = EM_IN2_PIN, uint8_t in3 = EM_IN3_PIN, uint8_t in4 = EM_IN4_PIN);
    ~StarterBot();
    IRremote  *mIrRecv;
    PS2X *mPs2x;
    ...
    void IrInit(void);
	void SetUltrasonicpin(uint8_t Echo_Pin = EM_ECHO_PIN, uint8_t Trig_Pin = EM_TRIG_PIN);
    void SetServoPin(uint8_t Servo_Pin = EM_SERVO_PIN);
    ...
    void SetUltrasonicPin(uint8_t Echo_Pin = EM_ECHO_PIN, uint8_t Trig_Pin = EM_TRIG_PIN, uint8_t Servo_Pin = EM_SERVO_PIN);
